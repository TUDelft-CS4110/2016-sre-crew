...
Therefore we decided to shrink down the alphabet to the most meaningful words in term of states of the system.
...
2. The application was using a SQLite database to store strings that were inserted by the user when creating a new group ([Figure 8](#Figure_8)). The problem in this case was that SageMath was using a `Dialog` layout to insert the new user string. Therefore, the `OK` button could not be retrieved through the XML layout file. We used the fuzzing tool to insert random input into the field and then called  the `clickSpecific` function for the `OK` button. After various text inputs, the application crashed when a large text was inserted. This occurred because the text was not inserted into the database cause of its size. As a result the reference towards that text was `null` and led to a crash when the application tried to display it.
